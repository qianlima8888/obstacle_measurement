...
Mat image_laser = cv::Mat::zeros( 440, 440, CV_8UC1 );
...
void houghlinedetect(Mat& roiImg, string objectName)
{
	...
	for (int i = 0; i < H_Line.size(); ++i) {//查找水平边缘线
		if (H_Line[i][2].y < H_Line[top][2].y) top = i;
    ...
    //绘制轮廓
	Point crossPoint = getCrossPoint(paramA, paramB);
	line(gray_dst, H_Line[top][0], crossPoint, Scalar(0, 0, 255), 1, LINE_AA);
	line(gray_dst, V_Line[left][0], crossPoint, Scalar(0, 0, 255), 1, LINE_AA);

	crossPoint = getCrossPoint(paramA, paramD);
	line(gray_dst, H_Line[top][0], crossPoint, Scalar(0, 0, 255), 1, LINE_AA);
	line(gray_dst, V_Line[right][0], crossPoint, Scalar(0, 0, 255), 1, LINE_AA);

	crossPoint = getCrossPoint(paramC, paramB);
	line(gray_dst, H_Line[bottom][0], crossPoint, Scalar(0, 0, 255), 1, LINE_AA);
	line(gray_dst, V_Line[left][0], crossPoint, Scalar(0, 0, 255), 1, LINE_AA);

	crossPoint = getCrossPoint(paramC, paramD);
	line(gray_dst, H_Line[bottom][0], crossPoint, Scalar(0, 0, 255), 1, LINE_AA);
	line(gray_dst, V_Line[right][0], crossPoint, Scalar(0, 0, 255), 1, LINE_AA);
...
void laser_to_rgb( const sensor_msgs::LaserScanConstPtr& scan, vector<laser_coor>& laserPoint )
{
		...
		for(int id = 5; id<laserPoint.size()-5; id++)
		{
			...
			if(isEdgePoint(laserPoint[id-5].second, laserPoint[id].second, laserPoint[id+5].second, 4))
			{
...
vector<laser_coor> getIndex(vector<laser_coor>& Point)
{
	...
	do{
		...
		maxP = end - begin;
		if(it == Point.end()) break;
		lastIt = it;
		it = find_if(it+1, Point.end(), [](laser_coor x){return x.first.second;});
		
	}while(true);

	if(maxP>8) return vector<laser_coor>(Point.begin()+begin, Point.begin()+end);
	else return vector<laser_coor>(0);
...
void combineCallback(const sensor_msgs::ImageConstPtr& rgb_image_qhd, const sensor_msgs::LaserScanConstPtr& laser_data )
{
	...
	for( ; new_detection_iterator != detection_record_new.end(); new_detection_iterator++, detection_i++ )
	{
		...
		if((x + w) > rgb_ptr->image.cols) w = rgb_ptr->image.cols - x;
		if((y + h) > rgb_ptr->image.rows) h = rgb_ptr->image.rows - y;
        ...
        //抠图 去除背景干扰
        Mat cut, bg, fg;
		grabCut(rgb_ptr->image, cut, object_rect, bg, fg, 1, GC_INIT_WITH_RECT);
