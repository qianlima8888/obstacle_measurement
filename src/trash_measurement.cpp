...
void measurement(Mat &roiImg, vector<laser_coor> &laserPoint, int label, int x, int w)
{
	...
	circle(LaserMat, crossPointTL, 2, Scalar(0, 255, 255), 2, 1);
	circle(LaserMat, crossPointTR, 2, Scalar(0, 255, 255), 2, 1);
	circle(LaserMat, crossPointBL, 2, Scalar(0, 255, 255), 2, 1);
	circle(LaserMat, crossPointBR, 2, Scalar(0, 255, 255), 2, 1);
...
void pointInRoi(Rect roi, vector<laser_coor> &allPoint, vector<laser_coor> &inPoint)
{
	...
	{
        if( (pow((inPoint[i].second.x - inPoint[i-1].second.x), 2) + pow((inPoint[i].second.y - inPoint[i-1].second.y), 2)) > (maxDis * maxDis) )
		{
			begin = i;
	...
	for (; new_detection_iterator != detection_record_new.end(); new_detection_iterator++, detection_i++)
	{
		...
		try
		{
			...
			for (int i = 0; i < inPoint.size(); i++)
			{
				...
				circle(LaserMat, inPoint[i].first.first, 2, Scalar(0, 0, 255), 2, 1); //红色显示边缘点
